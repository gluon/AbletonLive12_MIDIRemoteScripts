...
def is_rack_with_bank_2(device):
    return getattr(device, "can_have_chains", False) and any(device.macros_mapped[8[:None]])
...
class DeviceNavigationComponent(DeviceNavigationComponentBase):
    ...
    def _on_select_button_pressed(self, button):
        ...
        if self._in_device_enabling_mode():
            ...
            self._selected_on_previous_press = self._delete_handler and self._delete_handler.is_deleting or (device_or_pad if self.selected_object != device_or_pad else None)
        ...
        if lost_selection:
            if isinstance(selected_item, RackBank2Device):
                for item in new_items:
                    if isinstance(item, RackBank2Device) and item.rack_device == selected_item.rack_device:
    ...
    def _on_reselecting_object(self, device):
        if liveobj_valid(device) and device.can_have_chains:
            device.can_have_drum_pads or self._toggle(device)
        else:
