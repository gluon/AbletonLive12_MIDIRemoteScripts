...
# Embedded file name: output/Live/mac_universal_64_static/Release/python-bundle/MIDI Remote Scripts/Push2/track_list.py
...
class TrackListComponent(ModesComponent, Messenger):
    ...
    def __init__(self, tracks_provider=None, trigger_recording_on_release_callback=nop, color_chooser=None, clip_phase_enabler=None, *a, **k):
        ...
        self._track_selected_when_pressed = [
         False] * self.track_action_buttons.control_count
        ...
        self._TrackListComponent__on_clip_phase_enabler_changed.subject = self.clip_phase_enabler
        ...
        self._setup_action_mode("mute",
          handler=partial(toggle_mixable_mute, song=(self.song)))
        self._setup_action_mode("solo",
          handler=partial(toggle_mixable_solo, song=(self.song)))
        self._setup_action_mode("stop",
          handler=(self._stop_track_clip),
          feedback_provider=stop_clip_button_color)
        self._setup_action_mode("select_color",
          handler=(self._select_mixable_color),
          exit_handler=(partial(self._select_mixable_color, None)))
        ...
        self._TrackListComponent__on_items_changed.subject = self._track_provider
        self._TrackListComponent__on_selected_item_changed.subject = self._track_provider
        self._TrackListComponent__on_tracks_changed.subject = self.song
        self._TrackListComponent__on_selected_track_changed.subject = self.song.view
        self._TrackListComponent__on_is_playing_changed.subject = self.song
    ...
    def _setup_action_mode(self, name, handler, exit_handler=nop, feedback_provider=mixable_button_color):
        self.add_mode(name,
          [
         (
          partial((self._enter_action_mode),
            handler=handler,
            feedback_provider=feedback_provider),
          exit_handler)],
          behaviour=(TrackListBehaviour()))
    ...
    def _update_track_and_chain_listeners(self):
        self.notify_tracks()
        tracks = self.tracks
        self._TrackListComponent__on_track_color_index_changed.replace_subjects(tracks)
        self._TrackListComponent__on_track_mute_state_changed.replace_subjects(tracks)
        self._TrackListComponent__on_track_muted_via_solo_changed.replace_subjects(tracks)
        self._TrackListComponent__on_track_solo_state_changed.replace_subjects(tracks)
        tracks_without_chains = list(filter(can_play_clips, tracks))
        self._TrackListComponent__on_track_fired_slot_changed.replace_subjects(tracks_without_chains)
        self._TrackListComponent__on_track_playing_slot_changed.replace_subjects(tracks_without_chains)
        self._TrackListComponent__on_track_is_frozen_state_changed.replace_subjects(tracks_without_chains)
    ...
    def track_action_buttons(self, button):
        if self.selected_mode == "select":
            if self._track_selected_when_pressed[button.index]:
    ...
    def _select_mixable(self, track):
        if liveobj_valid(track):
            if liveobj_changed(self._track_provider.selected_item, track):
