...
# Embedded file name: output/Live/mac_universal_64_static/Release/python-bundle/MIDI Remote Scripts/novation/launchpad_elements.py
...
def create_slider(identifier, name, **k):
 ...
 MIDI_CC_TYPE, BUTTON_FADER_MAIN_CHANNEL, identifier, name=name, **k)
...
class LaunchpadElements(object):
    ...
    def __init__(self, arrow_button_identifiers=(91, 92, 93, 94), session_mode_button_identifier=95, *a, **k):
        ...
        self.up_button = create_button(arrow_button_identifiers[0], "Up_Button")
        self.down_button = create_button(arrow_button_identifiers[1], "Down_Button")
        self.left_button = create_button(arrow_button_identifiers[2], "Left_Button")
        self.right_button = create_button(arrow_button_identifiers[3], "Right_Button")
        self.session_mode_button = create_button(session_mode_button_identifier, "Session_Mode_Button")
        self.scene_launch_buttons_raw = [create_button(identifier, "Scene_Launch_Button_{}".format(row_index)) for row_index, identifier in enumerate(range(89, 18, -10))]
        self.scene_launch_buttons = ButtonMatrixElement(rows=[
         self.scene_launch_buttons_raw],
          name="Scene_Launch_Buttons")
        self.clip_launch_matrix = ButtonMatrixElement(rows=[[create_button((offset + col_index), ("{}_Clip_Launch_Button_{}".format(col_index, row_index)), msg_type=MIDI_NOTE_TYPE) for col_index in range(SESSION_WIDTH)] for row_index, offset in enumerate(range(81, 10, -10))],
    ...
    def _create_drum_pads(self):
        drum_pad_rows = ((64, 65, 66, 67, 96, 97, 98, 99), (60, 61, 62, 63, 92, 93, 94, 95),
                         (56, 57, 58, 59, 88, 89, 90, 91), (52, 53, 54, 55, 84, 85, 86, 87),
                         (48, 49, 50, 51, 80, 81, 82, 83), (44, 45, 46, 47, 76, 77, 78, 79),
                         (40, 41, 42, 43, 72, 73, 74, 75), (36, 37, 38, 39, 68, 69, 70, 71))
        self.drum_pads = ButtonMatrixElement(rows=[[create_button((row_identifiers[col_index]), ("Drum_Pad_{}_{}".format(col_index, row_index)), msg_type=MIDI_NOTE_TYPE, channel=8) for col_index in range(SESSION_WIDTH)] for row_index, row_identifiers in enumerate(drum_pad_rows)],
          name="Drum_Pads")

    def _create_scale_pads(self):
        self.scale_pads = ButtonMatrixElement(rows=[
         [create_button(identifier, ("Scale_Pad_{}".format(identifier)), msg_type=MIDI_NOTE_TYPE, channel=15) for identifier in range(128)]],
          name="Scale_Pads")
