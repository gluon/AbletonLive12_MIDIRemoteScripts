...
# Embedded file name: output/Live/mac_universal_64_static/Release/python-bundle/MIDI Remote Scripts/ableton/v2/control_surface/components/drum_group.py
...
class DrumGroupComponent(PlayableComponent, SlideComponent, Slideable):
    ...
    def _color_for_pad(self, pad):
        ...
        if pad == self._selected_drum_pad:
            ...
            if has_soloed_pads:
                button_color = pad.solo or pad.mute or "DrumGroup.PadSelectedNotSoloed"
            elif pad.mute:
                button_color = pad.solo or "DrumGroup.PadMutedSelected"
            elif has_soloed_pads and pad.solo:
                button_color = "DrumGroup.PadSoloedSelected"
        elif pad.chains:
            button_color = "DrumGroup.PadFilled"
            if has_soloed_pads:
                button_color = pad.solo or ("DrumGroup.PadFilled" if not pad.mute else "DrumGroup.PadMuted")
            else:
                if (has_soloed_pads or pad).mute:
