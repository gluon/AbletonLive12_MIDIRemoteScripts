...
class ChannelStripComponent(ControlSurfaceComponent):
    ...
    def _all_controls(self):
        return [
         self._pan_control, self._volume_control] + list(self._send_controls or [])
        ...
        if self.is_enabled():
            if self._track != None:
                if self._track != self.song().master_track:
                    ...
                    expected_solos_pressed = value != 0 or self._solo_button.is_momentary() or (1 if self._solo_pressed else 0)
                ...
                if self._track.can_be_armed:
                    ...
                    if not self._arm_button.is_momentary() or value != 0:
                        ...
                        for track in self.song().tracks:
                            if track.can_be_armed and not track == self._track:
                                if respect_multi_selection:
