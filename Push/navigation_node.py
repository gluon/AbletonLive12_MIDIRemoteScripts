...
# Embedded file name: output/Live/mac_universal_64_static/Release/python-bundle/MIDI Remote Scripts/Push/navigation_node.py
                ...
                if isinstance(model_object, Live.Chain.Chain):
                    ...
                    if isinstance(model_object, Live.DrumPad.DrumPad):
                        ...
                        if isinstance(model_object, Live.Device.Device):
                            ...
                            if node.parent:
                                node.children or node.disconnect()
                                node = None
...
class ModelNode(NavigationNode):
    ...
    def _get_parent_with_class(self, cls):
        ...
        while liveobj_valid(node):
            node = isinstance(node, cls) or node.canonical_parent
...
class ChainNode(ModelNode):
            ...
            if index >= 0:
                if index < len(self._children):
                    if not isinstance(self._children[index][1], Live.DrumPad.DrumPad):
                        drumpads_before = len(list(filter((lambda _ChainNode__x: isinstance(_ChainNode__x[1], Live.DrumPad.DrumPad)), self._children[None[:index]])))
    ...
    def _get_children_from_model(self):
        ...
        def expand_device(d):
            is_rack_with_bank_2 = getattr(d, "can_have_chains", False) and any(d.macros_mapped[8[:None]])
