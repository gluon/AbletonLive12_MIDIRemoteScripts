...
# Embedded file name: output/Live/mac_universal_64_static/Release/python-bundle/MIDI Remote Scripts/APC40/APC40.py
# Compiled at: 2024-03-09 01:30:22
# Size of source mod 2**32: 15468 bytes
...
class APC40(APC, OptimizedControlSurface):
    ...
    def _create_controls(self):
        ...
        self._shift_button = make_button(0,
          98, resource_type=PrioritizedResource, name="Shift_Button")
        ...
        self._scene_launch_buttons_raw = [make_color_button(0, (index + 82), name=("Scene_%d_Launch_Button" % index)) for index in range(SESSION_HEIGHT)]
        ...
        self._matrix_rows_raw = [[make_color_button(track_index, (scene_index + 53), name=("%d_Clip_%d_Button" % (track_index, scene_index))) for track_index in range(SESSION_WIDTH)] for scene_index in range(SESSION_HEIGHT)]
        for row in self._matrix_rows_raw:
            self._session_matrix.add_row(row)

        self._selected_slot_launch_button = make_pedal_button(67,
          name="Selected_Slot_Launch_Button")
        self._selected_scene_launch_button = make_pedal_button(64,
          name="Selected_Scene_Launch_Button")
        ...
        self._master_select_button = make_on_off_button(0,
          80, name="Master_Select_Button")
        ...
        bank_button_labels = ('Clip_Track_Button', 'Device_On_Off_Button', 'Previous_Device_Button',
                              'Next_Device_Button', 'Detail_View_Button', 'Rec_Quantization_Button',
        ...
        self._global_bank_buttons = [make_on_off_button(0, (87 + index), name=name) for index, name in enumerate(('Pan_Button',
                                                                                                                  'Send_A_Button',
                                                                                                                  'Send_B_Button',
                                                                                                                  'Send_C_Button'))]
    ...
    def _create_session(self):
        self._session = SessionComponent(SESSION_WIDTH,
          ...
          selected_scene_launch_button=(self._selected_scene_launch_button)))
    ...
    def _create_detail_view_control(self):
        self._detail_view_toggler = DetailViewCntrlComponent(name="Detail_View_Control",
          is_enabled=False,
          layer=Layer(device_clip_toggle_button=(self._device_clip_toggle_button),
    ...
    def _create_fallback_control_owner(self):
        self.register_disconnectable(SimpleLayerOwner(layer=Layer(_matrix=(self._session_matrix),
