...
# Embedded file name: output/Live/mac_universal_64_static/Release/python-bundle/MIDI Remote Scripts/_APC/APC.py
...
class APC(ControlSurface):
    ...
    def handle_sysexParse error at or near `COME_FROM' instruction at offset 66_1
        ...
        if midi_bytes[5] == MANUFACTURER_ID:
            if midi_bytes[6] == self._product_model_id_byte():
                if self._identity_response_pending:
                    ...
                    self._log_versionversion_bytes
    ...
    def _log_version(self, version_bytes):
        ...
        self.log_messagemessage

    def _activate_combination_mode(self, track_offset, support_devices):
        self.highlighting_session_component().link_with_track_offsettrack_offset
    ...
    def _do_uncombine(self):
        if self in APC._active_instances:
            APC._active_instances.removeself
