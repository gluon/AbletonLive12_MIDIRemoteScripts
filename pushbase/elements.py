...
# Embedded file name: output/Live/mac_universal_64_static/Release/python-bundle/MIDI Remote Scripts/pushbase/elements.py
...
class Elements(object):
    ...
    def __init__(self, deleter=None, undo_handler=None, pad_sensitivity_update=None, playhead=None, velocity_levels=None, continuous_mapping_sensitivity=None, fine_grained_continuous_mapping_sensitivity=None, full_velocity=None, *a, **k):
        ...
        self.delete_button = create_modifier_button(118,
          "Delete_Button", undo_step_handler=undo_handler)
        self.duplicate_button = create_modifier_button(88,
          "Duplicate_Button", undo_step_handler=undo_handler)
        self.quantize_button = create_modifier_button(116,
          "Quantization_Button", undo_step_handler=undo_handler)
        ...
        self.pan_send_mix_mode_button = create_button(115,
          "Pan_Send_Mode_Button", resource_type=PrioritizedResource)
        ...
        self.create_device_button = create_button(52,
          "Create_Device_Button", undo_step_handler=undo_handler)
        self.create_track_button = create_button(53,
          "Create_Track_Button", undo_step_handler=undo_handler)
        self.double_button = create_button(117,
          "Double_Button", undo_step_handler=undo_handler)
        self.user_button = create_button(59,
          "User_Button", undo_step_handler=undo_handler)
        self.select_buttons_raw = [create_button(20 + idx, "Track_Select_Button" + str(idx)) for idx in range(8)]
        self.select_buttons = ButtonMatrixElement(name="Track_Select_Buttons",
          rows=[self.select_buttons_raw])
        self.track_state_buttons_raw = [create_button((102 + idx), ("Track_State_Button" + str(idx)), is_rgb=True) for idx in range(8)]
        self.track_state_buttons = ButtonMatrixElement(name="Track_State_Buttons",
          rows=[self.track_state_buttons_raw])
        self.side_buttons_raw = [create_button(36 + idx, "Scene_Launch_Button" + str(idx)) for idx in reversed(range(8))]
 ...
 36 + pad_id, skin=skin, 
        ...
        self.matrix_rows_raw = [[create_pad_button(((7 - row) * 8 + column), (str(column) + "_Clip_" + str(row) + "_Button"), is_rgb=True) for column in range(8)] for row in range(8)]
        ...
        self.swing_control = TouchEncoderElement(channel=0,
          identifier=15,
          map_mode=(consts.GLOBAL_MAP_MODE),
          name="Swing_Control",
          undo_step_handler=undo_handler,
        ...
        self.master_volume_control = TouchEncoderElement(channel=0,
          identifier=79,
          map_mode=(consts.GLOBAL_MAP_MODE),
          undo_step_handler=undo_handler,
          delete_handler=deleter,
        ...
        self.fine_grain_param_controls = ButtonMatrixElement(rows=[
