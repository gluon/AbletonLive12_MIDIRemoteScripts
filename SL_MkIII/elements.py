...
# Embedded file name: output/Live/mac_universal_64_static/Release/python-bundle/MIDI Remote Scripts/SL_MkIII/elements.py
...
def create_display_color_element_line(v_position, **k):
    raw_line = [create_display_color_element(index, v_position, ("Color_Field_{}_{}".format(index, v_position)), **k) for index in range(SESSION_WIDTH)]
    return (
     raw_line,
     ButtonMatrixElement(rows=[
      raw_line],
       name=("Color_Field_Line_{}".format(v_position))))
...
def create_selection_field_line(v_position):
    ...
    return (
     ...
     ButtonMatrixElement(rows=[
      lines_raw],
       name=("Selection_Field_Line_{}".format(v_position))))
...
def create_text_display_line(v_position):
    display = mixin(CachingControlElement, ConfigurablePhysicalDisplayElement)(v_position=v_position,
...
class Elements:
    ...
    def __init__(self, *a, **k):
        ...
        pads_raw = [[create_button((offset + col_index), ("Pad_{}_{}".format(col_index, row_index)), msg_type=MIDI_NOTE_TYPE) for col_index in range(SESSION_WIDTH)] for row_index, offset in enumerate((96,
                                                                                                                                                                                                         112))]
        ...
        self.pads_flattened = ButtonMatrixElement(rows=[
         pads_raw[0] + pads_raw[1]],
          name="Pads_Flattened")
        self.shifted_pad_row_1 = ButtonMatrixElement(rows=[
         [with_shift(control) for control in pads_raw[1]]],
          name="Pad_Row_1_With_Shift")
        scene_launch_buttons_raw = [create_button((83 + row_index), name=("Scene_Launch_Button_{}".format(row_index))) for row_index in range(SESSION_HEIGHT)]
        ...
        sliders = [SpecialFeedbackChannelSliderElement(MIDI_CC_TYPE, DEFAULT_CHANNEL, (41 + index), name=("Slider_{}".format(index))) for index in range(SESSION_WIDTH)]
        ...
        self.slider_leds = ButtonMatrixElement(rows=[
         [create_rgb_led(index + 54, "Slider_LED_{}".format(index)) for index in range(8)]],
          name="Slider_LEDs")
        self.mixer_soft_button_row_0 = ButtonMatrixElement(rows=[
         [create_button(identifier, "Mixer_Soft_Button_{}_0".format(col_index)) for col_index, identifier in enumerate(range(59, 59 + SESSION_WIDTH))]],
          name="Mixer_Soft_Button_Row_0")
        self.mixer_soft_button_row_1 = ButtonMatrixElement(rows=[
         [create_button(identifier, "Mixer_Soft_Button_{}_1".format(col_index)) for col_index, identifier in enumerate(range(59 + SESSION_WIDTH, 59 + SESSION_WIDTH * 2))]],
          name="Mixer_Soft_Button_Row_1")
        self.message_display = SpecialPhysicalDisplayElement(width_in_chars=38,
        ...
        self.center_display_1 = mixin(CachingControlElement, ConfigurablePhysicalDisplayElement)(v_position=0,
          width_in_chars=9,
          name="Center_Display_1")
        self.center_display_2 = mixin(CachingControlElement, ConfigurablePhysicalDisplayElement)(v_position=1,
          width_in_chars=9,
          name="Center_Display_2")
        self.mixer_display_1 = mixin(CachingControlElement, ConfigurablePhysicalDisplayElement)(v_position=2,
          width_in_chars=9,
          name="Mixer_Button_Display_1")
        self.mixer_display_2 = mixin(CachingControlElement, ConfigurablePhysicalDisplayElement)(v_position=3,
          width_in_chars=9,
        ...
        self.center_color_field = create_display_color_element(8, 0, "Center_Color_Field")
        self.mixer_color_field_1 = create_display_color_element(8, 1, "Mixer_Color_Field_1")
        self.mixer_color_field_2 = create_display_color_element(8, 2, "Mixer_Color_Field_2")
        self.select_buttons_raw = [create_button(51 + index, "Select_Button_{}".format(index)) for index in range(8)]
        ...
        self.selection_field_line_1_with_shift = ButtonMatrixElement(rows=[
         [with_shift(field) for field in self.selection_field_line_1_raw]],
          name="Selection_Field_Line_1_With_Shift")
        self.selection_field_line_2_with_shift = ButtonMatrixElement(rows=[
        ...
        self.encoder_color_fields = ButtonMatrixElement(rows=[
         [create_display_color_element(index, 1, "Encoder_Color_Field_{}".format(index)) for index in range(8)]],
