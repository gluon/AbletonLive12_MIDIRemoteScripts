...
# Embedded file name: output/Live/mac_universal_64_static/Release/python-bundle/MIDI Remote Scripts/_MxDCore/MxDCore.py
...
class MxDCore(object):
    ...
    def _get_lom_ids_of_mapped_objects(self, lom_id):
        observers = []
        remotes = []
        modulators = []
            ...
            for object_id, _ in device_context.items():
                ...
                if type == "obs":
                    if self._get_current_lom_id(device_id, object_id) == int(lom_id):
                        ...
                        if self._get_current_lom_id(device_id, object_id) == int(lom_id):
                            modulators.append((device_id, object_id))

        return (
         observers, remotes, modulators)
    ...
    def _set_current_lom_id(self, device_id, object_id, lom_id, type):
        ...
        if self.manager.set_current_lom_id(device_id, object_id, lom_id):
            ...
            if type == "obs":
                ...
                if type == "rmt":
                    self._update_timeable(device_id, object_id, 4, True)
                else:
                    if type == "mod":
    ...
    def _set_current_lom_id_from_param(self, device_id, object_id, object_type, parameter):
        if self._is_integer(parameter):
            if self._lom_id_exists(device_id, int(parameter)):
    ...
    def _get_current_type(self, device_id, object_id):
        ...
        if current_type != -1:
            return {
             0: '"obs"', 1: None, 2: '"obj"', 3: '"obs"', 4: '"rmt"', 
             5: '"mod"'}[current_type]
    ...
    def obj_set(self, device_id, object_id, parameters):
        ...
        if current_object != None:
            ...
            property_values = parsed_params[1[:None]]
    ...
    def _object_attr_path_iter(self, device_id, object_id, path_components):
        ...
        for component in path_components[1[:None]]:
            if cur_object == None:
                return
    ...
    def _goto_path(self, device_id, object_id, parameters):
        ...
        try:
            ...
            if tuple(path_components[None[:2]]) == (LIVE_APP, CONTROL_SURFACES):
                object_context[PATH_KEY] = [LIVE_APP, CONTROL_SURFACES]
                path_components = path_components[2[:None]]
            for parameter in path_components:
                if parameter == "up":
                    del object_context[PATH_KEY][-1]
                elif parameter in ROOT_KEYS:
                    object_context[PATH_KEY] = []
    ...
    def _object_default_call_handler(self, device_id, object_id, lom_object, parameters):
        ...
        result = function(*parameters[1[:None]])
    ...
    def _extract_dict_from_parameters(self, parameters):
        result = None
        if len(parameters) > 0:
            if isinstance(parameters[0], string_types):
                try:
    ...
    def _object_get_notes_by_id_handler(self, device_id, object_id, lom_object, parameters):
        function_name, function_parameters = parameters[0], parameters[1[:None]]
    ...
    def _object_note_handler(self, device_id, object_id, lom_object, parameters):
        ...
        try:
            ...
            operations[NOTE_BUFFER_KEY].append(note_from_parameters(parameters[1[:None]]))
    ...
    def update_lom_ids_of_mapped_objects(self, lom_id):
        observers, remotes, modulators = self._get_lom_ids_of_mapped_objects(lom_id)
        for device_id, object_id in observers:
            self._observer_update_listener(device_id, object_id)

        for device_id, object_id in remotes:
            self._update_timeable(device_id, object_id, 4, False)

        for device_id, object_id in modulators:
            self._update_timeable(device_id, object_id, 5, False)
