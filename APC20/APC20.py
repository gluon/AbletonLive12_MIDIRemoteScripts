...
# Embedded file name: output/Live/mac_universal_64_static/Release/python-bundle/MIDI Remote Scripts/APC20/APC20.py
...
class APC20(APC):
    ...
    def _create_controls(self):
        ...
        self._scene_launch_buttons = [make_color_button(0, (index + 82), name=("Scene_%d_Launch_Button" % index)) for index in range(SESSION_HEIGHT)]
        ...
        for scene_index in range(SESSION_HEIGHT):
            row = [make_color_button(track_index, (scene_index + 53), name=("%d_Clip_%d_Button" % (track_index, scene_index))) for track_index in range(SESSION_WIDTH)]
        ...
        self._scene_launch_buttons = ButtonMatrixElement(name="Scene_Launch_Buttons",
          rows=[self._scene_launch_buttons])
        self._solo_buttons = [make_button(track_index, 49, name=("%d_Solo_Button" % track_index)) for track_index in range(MIXER_SIZE)]
        self._mute_buttons = [make_button(track_index, 50, name=("%d_Mute_Button" % track_index)) for track_index in range(MIXER_SIZE)]
        ...
        self._select_buttons = [make_button(track_index, 51, name=("%d_Select_Button" % track_index)) for track_index in range(8)]
        self._arm_buttons = [make_button(track_index, 48, name=("%d_Arm_Button" % track_index)) for track_index in range(8)]
        self._sliders = [make_slider(track_index, 7, name=("%d_Volume_Control" % track_index)) for track_index in range(8)]
        self._note_matrix = ButtonMatrixElement(name="Note_Button_Matrix")
        self._note_buttons = [[make_button(9, (note + i), name=("Note_%d_Button" % (note + i))) for i in range(4)] for note in range(36, 75, 4)]
    ...
    def _create_session(self):
        ...
        self._session_zoom = ShiftableZoomingComponent((self._session),
