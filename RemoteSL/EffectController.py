...
class EffectController(RemoteSLComponent):
    ...
    def receive_midi_ccParse error at or near `COME_FROM' instruction at offset 138_6

    def receive_midi_noteParse error at or near `COME_FROM' instruction at offset 10_0
        ...
        if not self._EffectController__assigned_device == None:
            ...
            for s in self._EffectController__strips:
                ...
                if chosen_bank:
                    ...
                    device_parameters = self._EffectController__assigned_device.parameters[1[:None]]
                    device_parameters = self._EffectController__assigned_device.parameters[1[:None]]
            ...
            if cc_value == CC_VAL_BUTTON_PRESSED:
                if cc_no == FX_DISPLAY_PAGE_UP:
                    ...
                    if cc_no == FX_DISPLAY_PAGE_DOWN:
                        ...
                        self._EffectController__bank = self._EffectController__assigned_device_is_locked or new_bank
                        self._EffectController__reassign_strips()
            else:
                self._EffectController__assigned_device.store_chosen_bank(self._EffectController__parent.instance_identifier(), new_bank)

    def __handle_select_button_ccsParse error at or near `COME_FROM' instruction at offset 298_6
    ...
    def __report_bank(self):
        if self._EffectController__show_bank:
            self._EffectController__show_bank = False
            if self._EffectController__assigned_device.class_name in list(DEVICE_DICT.keys()):
                if self._EffectController__assigned_device.class_name in list(BANK_NAME_DICT.keys()):
                    bank_names = BANK_NAME_DICT[self._EffectController__assigned_device.class_name]
                    if bank_names:
                        if len(bank_names) > self._EffectController__bank:
                            bank_name = bank_names[self._EffectController__bank]
                            self._EffectController__show_bank_select(bank_name)
                        else:
                            self._EffectController__show_bank_select("Best of Parameters")
                else:
                    pass
            else:
